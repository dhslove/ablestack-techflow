...
def main() -> None:
    ...
    if report_pages < 4 or "Issue #69" not in report_text or "Post #384" not in report_text or "ValidationError" not in report_text or "16,000" not in report_text: raise RuntimeError("report PDF contract failed")
    ...
    if deck_pages != 5 or slides != 5 or notes != 5 or "#384" not in deck_text or "0.14.6" not in deck_text or "4,678" not in deck_text or "16,000" not in deck_text or "231" not in deck_text or "ABLESTACK Diplo" not in deck_text: raise RuntimeError(f"presentation contract failed pages={deck_pages} slides={slides} notes={notes}")
