...
5. Knowledge Base 본문은 제목 없이 `증상`, `원인`, `해결 방법`, `추가 고려사항`, `적용 버전` 순서로 작성한다.
6. Chat은 승인 수단이 아니라 답변 게시, Knowledge Base 게시, 처리 실패를 확인하는 관찰 채널로 사용한다.
7. 인프라 상태를 변경하는 Ops 작업의 사람 승인은 그대로 유지한다. 이번 결정은 Community 답변 게시에만 적용한다.
...
- 자동 게시와 KB 게시 이벤트를 Case 이력에 기록한다.
...
stateDiagram-v2
    ...
    RESOLVED --> RESOLVED: Knowledge Base 게시
