...
TechFlow Artifact 안전 상한은 일반 파일 1GiB, 압축파일 10GiB, 압축 항목 100개, 한 번의 질문에 연결되는 첨부자료 최대 5개입니다. Community나 Chat 자체의 전송 제한이 더 작으면 해당 서비스의 제한이 우선합니다. 큰 로그는 문제 발생 시각 전후와 오류 부분을 중심으로 줄여서 올리면 더 빠르고 정확하게 분석할 수 있습니다.

경로 탈출, 중첩 압축, 압축 폭탄, 실행 파일 위장 또는 비밀정보가 발견된 자료는 차단될 수 있습니다. 일부 첨부가 안전하게 처리되지 않더라도 나머지 질문은 가능한 범위에서 계속 분석합니다.

### 4.4 Chat에서 이미지·로그 보내기

TechFlowAssist Bot 대화에 질문과 이미지·로그를 함께 보내면 Bot이 파일을 실제 분석 대상으로 등록합니다. 설명 없이 파일만 보내도 접수되지만, 확인하고 싶은 증상이나 작업을 한 문장으로 함께 적는 것이 더 정확합니다.

같은 문제의 화면과 로그를 차례로 보내면 해결 표시 전까지 같은 대화 맥락에서 함께 검토합니다. 지원하지 않거나 손상된 파일은 분석한 것처럼 답하지 않고 지원 형식과 다시 올릴 방법을 Chat으로 안내합니다. PDF, Office 문서와 실행파일은 현재 Chat AI 분석 형식이 아닙니다.
