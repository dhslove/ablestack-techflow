...
| `RESOLVED` | 질문자가 해결 답변을 선택했고 KB 처리가 완료됨 | `상세`에서 최종 KB 링크 확인 |
...
자동 KB 생성을 시작하려면 **최초 질문을 작성한 사용자**가 해결 답변을 선택해야 합니다. 다른 참여자나 관리자가 대신 선택한 답변은 질문자의 해결 확인으로 처리되지 않습니다.
...
2. 질문자가 선택한 해결 답변과 실제 조치 결과를 중심으로 내용을 정리합니다.
...
| 솔루션을 선택했지만 KB가 없음 | 최초 질문자가 선택했는지 확인하고 `상세`, `이력` 조회. 계속 실패하면 Discussion ID와 Case ID를 운영 관리자에게 전달 |
